    ...
    public class GoogleShoppingService : BasePlugin, IMiscPlugin
    {
        ...
        public void GenerateFeed(Stream stream, Store store)
        {
            ...
            using (var writer = XmlWriter.Create(stream, settings))
            {
                ...
                //var ccc= _categoryService.get
                string[] splitIds= { };
                var excludeProducts = _settingService.GetSettingByKey<string>("custom.google.shopping.exclude.products");
                if (!string.IsNullOrEmpty(excludeProducts))
                {
                     splitIds = excludeProducts.Split(',');
                }
                ...
                foreach (var product1 in products1)
                {
                    ...
                    //var productCategories = _categoryService.GetProductCategoriesByProductId(product1.Id);
                    //foreach (var item in productCategories)
                    //{
                    //    //var ttt = splitIds.Where(x => x.Equals(item.CategoryId));
                    //    var results = Array.FindAll(splitIds, s => s.Equals(item.CategoryId.ToString()));
                    //    if (results.Length >= 1 )
                    //    {
                    //        catFound = true;
                    //        break;
                    //    }
                    //    //var eee = splitIds.Contains<string>(item.Id);
                    //}

                    string result = Array.Find(splitIds, element => element.Equals(product1.Sku));
                    if(result != null)
                    {
                        productFound = true;
                    }
                    ...
                    if (!productFound)
                    {
                        ...
                        foreach (var product in productsToProcess)
                        {
                            ...
                            string description = product.GetLocalized(x => x.ShortDescription, languageId);
                            ...
                            var pictures = _pictureService.GetProductPicturesByParentSku(product.Sku, countryId, _googleShoppingSettings.ProductPictureSize);
                            for (int i = 0; i < pictures.Count; i++)
                            {
                                ...
                                var imageUrl = picture.pictureurl;
