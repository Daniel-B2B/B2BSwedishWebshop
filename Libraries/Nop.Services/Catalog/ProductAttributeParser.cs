...
namespace Nop.Services.Catalog
{
    ...
    public partial class ProductAttributeParser : IProductAttributeParser
    {
        ...
        public virtual bool? IsConditionMet(ProductAttributeMapping pam, string selectedAttributesXml)
        {
            ...
            var depId = 0;
            var allFound = true;
            var valuesThatShouldBeSelected = new List<string>();
            var selectedValues = new List<string>();
            if (pam.ProductAttribute.Name.ToUpper() == "LOGOFIL" || pam.ProductAttribute.Name.ToUpper() == "NOTER")
            {
                var pa2 = _productAttributeService.GetProductAttributeMappingsByProductId(pam.Product.Id);
                var pa12 = pa2.Where(x => x.ProductAttribute.Name.ToUpper() == "TRYK").FirstOrDefault();
                depId = pa12.Id;

                valuesThatShouldBeSelected = ParseValues(conditionAttributeXml, depId)
                        //a workaround here:
                ...
                selectedValues = ParseValues(selectedAttributesXml, depId).ToList();

                valuesThatShouldBeSelected= new List<string>();
                   
                    var pa = _productAttributeService.GetProductAttributeMappingsByProductId(pam.Product.Id);
                    var pa1 = pa.Where(x => x.ProductAttribute.Name.ToUpper() == "TRYK").FirstOrDefault();
                    var tttt = pa1.ProductAttributeValues;
                    foreach (var yy in tttt.Skip(1))
                    {
                        valuesThatShouldBeSelected.Add(yy.Id.ToString());
                ...
                else
                {
                    ...
                    valuesThatShouldBeSelected = ParseValues(conditionAttributeXml, dependOnAttribute.Id)
                       //a workaround here:
                       //ConditionAttributeXml can contain "empty" values (nothing is selected)
                       //but in other cases (like below) we do not store empty values
                       //that's why we remove empty values here
                    ...
                    selectedValues = ParseValues(selectedAttributesXml, dependOnAttribute.Id).ToList();
                    ...
                    {
                        bool found = false;
                        foreach (var t2 in selectedValues)//if (t2 != "132702")                                                   // {
                            if (t1 == t2)
                                found = true;
                        //}
                        if (!found)
