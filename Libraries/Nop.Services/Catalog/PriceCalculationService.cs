    ...
    public partial class PriceCalculationService : IPriceCalculationService
    {
            ...
            out List<DiscountForCaching> appliedDiscounts)
        {
            ...
            var cachedPrice = _cacheManager.Get(cacheKey, cacheTime, () =>
            {
                ...
                var grossDiscountPricePercentage = _settingService.GetSettingByKey<decimal>("grossdiscount");

                if (grossDiscountPricePercentage != decimal.Zero && product.GrossNett.ToUpper() == "GROSS")
                {
                    price = price - price * (grossDiscountPricePercentage / 100);
                }
            ...
            out List<DiscountForCaching> appliedDiscounts)
        {
            ...
            var _customPriceClassService = EngineContext.Current.Resolve< ICustomPriceClassService>();
            var _customHandlingPriceClassService = EngineContext.Current.Resolve<IHandlingPriceClassService>();

            decimal finalPrice;

            var combination = _productAttributeParser.FindProductAttributeCombination(product, attributesXml);
            if (combination != null && combination.OverriddenPrice.HasValue && combination.OverriddenPrice!=decimal.Zero)
            {
            ...
            else
            {
                ...
                if (attributeValues != null)
                {
                    ...
                    {
                        if (attributeValue.ProductAttributeMapping.ProductAttribute.Name.ToUpper() == "TRYK" && attributeValue.Name != "Intet tryk"&& attributeValue.Name != "No pressure")
                        {
                            var selectedQuantity = quantity;
                            if (attributeValue.Name== "laserengraving") //lasergravering
                            {   
                                var cusHandlingPrice = _customHandlingPriceClassService.GetHandlingPriceBasedOnQuantity(quantity, attributeValue.ProductAttributeMapping.Handling);
                                cusHandlingPrice = cusHandlingPrice / quantity;
                                attributesTotalPrice += cusHandlingPrice;
                            ...
                            //}
                            //attributesTotalPrice += Convert.ToDecimal(attributeValue.ProductAttributeMapping.SetUpFee, CultureInfo.InvariantCulture) / quantity;
                            var resultString = System.Text.RegularExpressions.Regex.Match(attributeValue.Name, @"\d+").Value;
                            int pressColor = Int32.Parse(resultString);
                            var cusPrice = _customPriceClassService.GetPriceBasedOnQuantity(quantity, attributeValue.ProductAttributeMapping.PriceClass, pressColor);
                            attributesTotalPrice += cusPrice;

                            //attributesTotalPrice += get price from table with margin based on quantity
                        }
                        else
                        {
            ...
            var _settingService = EngineContext.Current.Resolve<Nop.Services.Configuration.ISettingService>();
            var siteMarginPricePercentage = _settingService.GetSettingByKey<decimal>("sitemargin");
            //var grossDiscountPricePercentage = _settingService.GetSettingByKey<decimal>("grossdiscount");

            //custom Akiko
            var productCategoryId = product.ProductCategories.FirstOrDefault();
            if (productCategoryId != null)
            {
                var catId = productCategoryId.CategoryId;
                var category = _categoryService.GetCategoryById(catId);
                if (!string.IsNullOrEmpty(category.Margin) && category.Margin!="0")
                {
                    decimal catMargin = Convert.ToDecimal(category.Margin, new System.Globalization.CultureInfo("en-US"));
                    finalPrice = finalPrice + finalPrice * (catMargin/100);
                }else if (siteMarginPricePercentage!= decimal.Zero)
                {
                    //decimal siteMargin = Convert.ToDecimal(siteMarginPricePercentage);
                    finalPrice = finalPrice + finalPrice * (siteMarginPricePercentage / 100);
                }
            }

            //if(grossDiscountPricePercentage != decimal.Zero && product.GrossNett.ToUpper() == "GROSS")
            //{
            //    finalPrice = finalPrice - finalPrice * (grossDiscountPricePercentage / 100);
            //}
            ...
            out int? maximumDiscountQty)
        {
            ...
            if (shoppingCartItem.Product.SetupCost > decimal.Zero)
            {
                //var _settingService = EngineContext.Current.Resolve<ISettingService>();             
                //string laserengraving = "laserengraving";
                //var laserengravingSetting = _settingService.GetSettingByKey<string>("custom.code.laserengraving");
                //if (!string.IsNullOrEmpty(laserengravingSetting))
                //{
                //    laserengraving = laserengravingSetting;
                //}
                decimal totalSetupCost = decimal.Zero;
                var attributeValues1 = _productAttributeParser.ParseProductAttributeValues(shoppingCartItem.AttributesXml);
                if(attributeValues1.Count() > 0)
                {
                    var imprintAttributes = attributeValues1.Where(x => x.ProductAttributeMapping.ProductAttribute.Name.ToUpper() == "TRYK").ToList();
                    foreach (var imprintAttribute in imprintAttributes)
                    {
                        if (imprintAttribute != null && imprintAttribute.Name != "laserengraving") //lasergravering
                        {
                            var resultString = System.Text.RegularExpressions.Regex.Match(imprintAttribute.Name, @"\d+").Value;
                            if (!string.IsNullOrEmpty(resultString))
                            {
                                int pressColor = Int32.Parse(resultString);
                                //decimal setupFee = Convert.ToDecimal(imprintAttribute.ProductAttributeMapping.SetUpFee);
                                totalSetupCost = totalSetupCost + (pressColor * shoppingCartItem.Product.SetupCost);
                            }
                        }
                    }
                    subTotal = subTotal+ totalSetupCost;

                    //var ttt = attributeValues1.Where(x => x.ProductAttributeMapping.ProductAttribute.Name.ToUpper() == "TRYK").FirstOrDefault();

                    //if (ttt != null && ttt.Name != "laserengraving") //lasergravering
                    //{
                    //    var resultString = System.Text.RegularExpressions.Regex.Match(ttt.Name, @"\d+").Value;
                    //    if (!string.IsNullOrEmpty(resultString))
                    //    {
                    //        int pressColor = Int32.Parse(resultString);

                    //        subTotal = subTotal + (pressColor * shoppingCartItem.Product.SetupCost);
                    //    }
            ...
            //decimal setupCost = shoppingCartItem.Product.SetupCost != 0 ? shoppingCartItem.Product.SetupCost : decimal.Zero;
            //return subTotal + setupCost;
            return subTotal;
        }
        ...
        public virtual decimal GetPriceClassValue(Product product,
           Customer customer,
           ShoppingCartType shoppingCartType,
           int quantity,
           string attributesXml,
                ...
                if (attributeValues != null)
                {
                    foreach (var attributeValue in attributeValues)
                    {
                        if (attributeValue.ProductAttributeMapping.ProductAttribute.Name.ToUpper() == "TRYK" && attributeValue.Name != "Intet tryk" && attributeValue.Name != "No pressure")
                        {
                            ...
                            if (attributeValue.Name == "laserengraving") //lasergravering
                            {
                                var cusHandlingPrice = _customHandlingPriceClassService.GetHandlingPriceBasedOnQuantity(quantity, product.Handling);
                                cusHandlingPrice = cusHandlingPrice / quantity;
                                attributesTotalPrice += cusHandlingPrice;
                            ...
                            var resultString = System.Text.RegularExpressions.Regex.Match(attributeValue.Name, @"\d+").Value;
                            int pressColor = Int32.Parse(resultString);
                            cusPrice = _customPriceClassService.GetPriceBasedOnQuantity(quantity, attributeValue.ProductAttributeMapping.PriceClass, pressColor);
                            attributesTotalPrice += cusPrice;
