    ...
    public partial class ProductController : BaseAdminController
    {
        ...
        public virtual ActionResult ProductList(DataSourceRequest command, ProductListModel model)
        {
            ...
            gridModel.Data = products.Select(x =>
            {
                ...
                var smallPics = _pictureService.GetProductPicturesByParentSku(x.Sku, CountryId, 1);
                if (smallPics.Count() > 0)
                {
                    productModel.PictureThumbnailUrl = smallPics.FirstOrDefault().pictureurl;
                }
                else
                {
                    //productModel.PictureThumbnailUrl = x.ExternalPictureURL;
                    //picture
