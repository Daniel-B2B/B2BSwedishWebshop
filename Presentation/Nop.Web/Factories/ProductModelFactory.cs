    ...
    public partial class ProductModelFactory : IProductModelFactory
    {
                        ...
                        if (!product.CustomerEntersPrice)
                        {
                            ...
                            else
                            {
                                ...
                                if (product.HasTierPrices)
                                {
                                    ...
                                    //var productCategoryId = product.maingroup.FirstOrDefault();// product.ProductCategories.FirstOrDefault();

                                    var catID = 0;
                                    //if (product.maingroup!=null && product.maingroup.Count() > 0)
                                    //{
                                    //    var catID1 = product.maingroup.FirstOrDefault();
                                    //    catID = Convert.ToInt32(catID1);
                                    //}
                                    //else if(product.ProductCategories.Count()>0)
                                    //{
                                       // catID = product.ProductCategories.FirstOrDefault().CategoryId;
                                    //}
                                    ...
                                    if(product.ProductCategories.Count() > 0)
                                    {
                                     ...
                                     if (catID > 0)
                                    {
                                        var catId = catID; // Convert.ToInt32(productCategoryId);// productCategoryId productCategoryId.CategoryId;
                                        ...
                                        if (!string.IsNullOrEmpty(category.Margin) && category.Margin != "0")
                                        {
                                            ...
                                            minPossiblePrice = minPossiblePrice + minPossiblePrice * (catMargin / 100);
                                        }
                                        else if (siteMarginPricePercentage != decimal.Zero)
                                        {
                                            //decimal siteMargin = Convert.ToDecimal(siteMarginPricePercentage);
                                            minPossiblePrice = minPossiblePrice + minPossiblePrice * (siteMarginPricePercentage / 100);
                                        }
                                    }
                                ...
                                priceModel.BasePricePAngV = product.FormatBasePrice(finalPrice,
                                    ...
                                    //priceModel.SetupCost = _priceFormatter.FormatPrice(product.SetupCost);
                                }
                        }
                    }
                    else
        ...
        protected virtual PictureModel PrepareProductOverviewPictureModel(Product product, int? productThumbPictureSize = null)
        {
            ...
            PictureModel defaultPictureModel = _cacheManager.Get(cacheKey, () =>
            {
                ...
                //var pics = _dbContext.SqlQuery<PictureModel>("select * from [dbo].[ProductPicturesByParentSKU](" + CountryId + ",'" + product.Sku + "')").ToList();
                //var smallPics = _pictureService.GetProductPicturesByParentSku(product.Sku, CountryId, 2);
                var normalPics = _pictureService.GetProductPicturesByParentSku(product.Sku, CountryId, 4);
                var largePics = _pictureService.GetProductPicturesByParentSku(product.Sku, CountryId, 5);
                ...
                if(normalPics.Count() > 0)
                {
                    pictureModel.ImageUrl = normalPics.FirstOrDefault().pictureurl;
                }
                else
                {
                    pictureModel.ImageUrl = product.ExternalPictureURL;
                }

                if (largePics.Count() > 0)
                {
                    pictureModel.FullSizeImageUrl = largePics.FirstOrDefault().pictureurl;
                }
                else
                {
                    pictureModel.FullSizeImageUrl = product.ExternalPictureURL;
                }

                //  ImageUrl = normalPics.FirstOrDefault().pictureurl,
                //FullSizeImageUrl = largePics.FirstOrDefault().pictureurl,
                //Title = string.Format(_localizationService.GetResource("Media.Product.ImageLinkTitleFormat"), productName),
                //AlternateText = string.Format(_localizationService.GetResource("Media.Product.ImageAlternateTextFormat"), productName)
        ...
        protected virtual IList<ProductDetailsModel.ProductAttributeModel> PrepareProductAttributeModels(Product product, ShoppingCartItem updatecartitem)
        {
            ...
            foreach (var attribute in productAttributeMapping)
            {
                var attributeModel = new ProductDetailsModel.ProductAttributeModel
                {
                    ...
                    Decription = attribute.AttributeDescription
        ...
        protected virtual IList<ProductDetailsModel.TierPriceModel> PrepareProductTierPriceModels(Product product)
        {
            ...
            decimal currentMargin = decimal.Zero;
            if (productCategoryId != null)
            {
                ...
                if (!string.IsNullOrEmpty(category.Margin) && category.Margin != "0")
                {                  
                    currentMargin = Convert.ToDecimal(category.Margin, new System.Globalization.CultureInfo("en-US"));
                    //finalPrice = finalPrice + finalPrice * (catMargin / 100);
                }
                else if (siteMarginPricePercentage != decimal.Zero)
                {                    
                    currentMargin = Convert.ToDecimal(siteMarginPricePercentage, new System.Globalization.CultureInfo("en-US"));
            ...
            var model = product.TierPrices.OrderBy(x => x.Quantity)
                   ...
                   .Select(tierPrice =>
                   {
                       ...
                       price = price + price * (currentMargin / 100);//Akiko
        ...
        protected virtual dynamic PrepareProductDetailsPictureModel(Product product, bool isAssociatedProduct = false)
        {
            ...
            var cachedPictures = _cacheManager.Get(productPicturesCacheKey, () =>
            {
                ...
                //var defaultPicture = pics.FirstOrDefault(); // pictures.FirstOrDefault();
                var defaultPictureModel = new PictureModel
                {
                    ImageUrl = normalPics.Count()>0? normalPics.FirstOrDefault().pictureurl:product.ExternalPictureURL,   // defaultPicture.pictureurl, // _pictureService.GetPictureUrl(defaultPicture, defaultPictureSize, !isAssociatedProduct),
                    FullSizeImageUrl = largePics.Count()>0? largePics.FirstOrDefault().pictureurl: product.ExternalPictureURL  // defaultPicture.pictureurl // _pictureService.GetPictureUrl(defaultPicture, 0, !isAssociatedProduct)
                };
                ...
                foreach (var picture in largePics) //pictures
                {
                    var pictureModel = new PictureModel
                    {
                        ...
                        ImageUrl = normalPics.Where(x=>x.ColorName==picture.ColorName).FirstOrDefault().pictureurl,
                        ThumbImageUrl = smallPics.Where(x => x.ColorName == picture.ColorName).FirstOrDefault().pictureurl,
                        FullSizeImageUrl = picture.pictureurl,
                    ...
                    pictureModel.Title = string.Format(_localizationService.GetResource("Media.Product.ImageLinkTitleFormat.Details"), productName);
                    pictureModel.AlternateText= string.Format(_localizationService.GetResource("Media.Product.ImageAlternateTextFormat.Details"), productName);
                    //"title" attribute
            ...
            //var smallPics1 = _pictureService.GetProductPicturesByParentSku(product.Sku, 3, 2);
            //var normalPics1 = _pictureService.GetProductPicturesByParentSku(product.Sku, 3, 4);
            //var largePics1 = _pictureService.GetProductPicturesByParentSku(product.Sku, 3, 5);
            ...
            ShoppingCartItem updatecartitem = null, bool isAssociatedProduct = false)
        {
            ...
            var model = new ProductDetailsModel
            {
                ...
                IsGroupBuyProduct = product.IsGroupBuyProduct////temp*
            };
