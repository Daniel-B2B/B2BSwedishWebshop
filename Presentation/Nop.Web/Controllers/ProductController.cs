...
namespace Nop.Web.Controllers
{
    public partial class ProductController : BasePublicController
    {
        ...
        #region Home Page Menu Product Image

        public virtual ActionResult GetHomePageProductImage(int categoryId)
        {
            var model = new HomePageMenuProductViewModel();
            if (categoryId != 0)
            {
                ...
                if (currentCategory != null && !string.IsNullOrEmpty(currentCategory.ProductSku))
                {
                    model.CustomText = currentCategory.CustomText;
                    var productData = _productService.GetProductBySku(currentCategory.ProductSku);
                    if (productData != null)
                    {
                        decimal taxRate;
                        var minPossiblePrice = _priceCalculationService.GetFinalPrice(productData, _workContext.CurrentCustomer);
                        if (productData.HasTierPrices)
                        {
                            //calculate price for the maximum quantity if we have tier prices, and choose minimal
                            minPossiblePrice = Math.Min(minPossiblePrice,
                                _priceCalculationService.GetFinalPrice(productData, _workContext.CurrentCustomer, quantity: int.MaxValue));
                        }
                       
                        decimal finalPriceBase = _taxService.GetProductPrice(productData, minPossiblePrice, out taxRate);
                        decimal finalPrice = _currencyService.ConvertFromPrimaryStoreCurrency(finalPriceBase, _workContext.WorkingCurrency);
                        model.Price = _priceFormatter.FormatPrice(finalPrice);
                        model.Seoname = productData.GetSeName();                      
                        var productPictures = _pictureService.GetProductPicturesByParentSku(productData.Sku, countryId, 3);
                        if (productPictures != null && productPictures.Count() > 0)
                        {
                            var pictureDefaultSizeUrlObj = productPictures.FirstOrDefault();
                            model.PictureUrl = pictureDefaultSizeUrlObj.pictureurl;
                        }
                        else
                        {
                            model.PictureUrl = productData.ExternalPictureURL;
                        }
                    }
                }
            }
