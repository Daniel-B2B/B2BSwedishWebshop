...
namespace Nop.Web.Controllers
{
    public partial class ShoppingCartController : BasePublicController
    {
            ...
            bool loadPicture, FormCollection form)
        {
            ...
            var qty = 1;
            var subtotal= "";
            var subtotalWithoutFormating = decimal.Zero;
            //decimal priceClassValue = decimal.Zero;
            var enteredQuantity = form["addtocart_"+ productId +".EnteredQuantity"];
            if (!string.IsNullOrEmpty(enteredQuantity) ) {
                qty = Convert.ToInt32(enteredQuantity);
            }
            ...
            //var ttt = _productAttributeParser.ParseProductAttributeValues(attributeXml, 16186);
            ...
            if (_permissionService.Authorize(StandardPermissionProvider.DisplayPrices) && !product.CustomerEntersPrice)
            {
                ...
                subtotalWithoutFormating = finalPriceWithDiscount * qty;
                subtotal = _priceFormatter.FormatPrice(subtotalWithoutFormating);
            ...
            //validateAttributeConditions = true;//Akiko
            if (validateAttributeConditions)
            {
                ...
                {
                    /* AKiko custom code  */
                    //if (attribute.ProductAttribute.Name.ToUpper() == "LOGOFIL" || attribute.ProductAttribute.Name.ToUpper() == "NOTER")
                    //{
                    //    var pressAttributelist = attributes.Where(x => x.ProductAttribute.Name.ToUpper() == "TRYK");
                    //    if (pressAttributelist != null && pressAttributelist.Count() > 0)
                    //    {
                    //        var selectedPressAttribute = _productAttributeParser.ParseValues(attributeXml, pressAttributelist.FirstOrDefault().Id);
                    //        var defaultPressAttribute = pressAttributelist.FirstOrDefault().ProductAttributeValues.Where(x => x.IsPreSelected).FirstOrDefault().Id.ToString();
                    //        if (selectedPressAttribute.FirstOrDefault() == defaultPressAttribute)
                    //        {
            ...
            //    //var _combination = EngineContext.Current.Resolve<IproductAttributeC>();
            //    var combination = _productAttributeService.GetAllProductAttributeCombinations(product.Id);
            //    if (combination != null)
            //    {
            //        sku = combination.FirstOrDefault().Sku;
            //    }
            //}


            int countryId = Convert.ToInt32(ConfigurationManager.AppSettings["CountryId"]);
            var pictureFullSizeUrlObj = _pictureService.GetProductPictureBySku(sku, countryId, 5);
            var pictureDefaultSizeUrlObj = _pictureService.GetProductPictureBySku(sku, countryId, 4);
            var colorPictures = new CustomPicture();
            //if no picture url then try for custom child sku - akiko
            if (pictureFullSizeUrlObj == null)
            {
                var attributeValues = _productAttributeParser.ParseProductAttributeValues(attributeXml);
                if (attributeValues != null && attributeValues.Count() > 0)
                {
                    var foundChildSku =  attributeValues.Where(x => !string.IsNullOrEmpty(x.PictureUrl)).FirstOrDefault();
                    pictureFullSizeUrlObj = _pictureService.GetProductPictureBySku(foundChildSku?.PictureUrl, countryId, 5);
                    pictureDefaultSizeUrlObj = _pictureService.GetProductPictureBySku(foundChildSku?.PictureUrl, countryId, 4);

                    colorPictures = _pictureService.GetProductPictureBySku(foundChildSku?.PictureUrl, countryId, 5);
                }
            }
            if(pictureFullSizeUrlObj!= null)
            {
                pictureFullSizeUrl = pictureFullSizeUrlObj.pictureurl;
            }
            else
            {
                pictureFullSizeUrl = product.ExternalPictureURL;
            }
            if (pictureDefaultSizeUrlObj != null)
            {
                pictureDefaultSizeUrl = pictureDefaultSizeUrlObj.pictureurl;
            }
            else
            {
                pictureDefaultSizeUrl = product.ExternalPictureURL;
            }

            if (product.SetupCost > decimal.Zero)
            {
                decimal totalSetupCost = decimal.Zero;
                var attributeValues1 = _productAttributeParser.ParseProductAttributeValues(attributeXml);
                if (attributeValues1.Count() > 0)
                {
                    ...
                    foreach (var imprintAttribute in imprintAttributes)
                    {
                        if (imprintAttribute != null && imprintAttribute.Name != "laserengraving") //lasergravering
                        {
                            ...
                            if (!string.IsNullOrEmpty(resultString))
                            {
                                ...
                                totalSetupCost = totalSetupCost + (pressColor * product.SetupCost);
                            }
                        }
                    }
                    subtotal = _priceFormatter.FormatPrice(subtotalWithoutFormating + totalSetupCost);
                    ...
                    //        subtotal = _priceFormatter.FormatPrice(subtotalWithoutFormating + (pressColor * product.SetupCost));
                    //    }
            ...
            return Json(new
            {
				...
				subtotal = subtotal,
                colorPictures=colorPictures
                //setupcosttext = setupCostText
            });
        }
